...
int thread_run_code(void *_arg) {
    ...
    while (__sync_bool_compare_and_swap(arg, 0, 1));
    log("1");
    log("2");
    log("3");
    *arg &= 0;
...
test_compare_and_swap_module_init(void) {
    run_assign_10();
    return 0;
}
...
test_compare_and_swap_module_cleanup(void) {
    log("Module that tests the 'compare_and_swap' in linux kernel.\n"
...
module_init(test_compare_and_swap_module_init);
module_exit(test_compare_and_swap_module_cleanup);
